    ...
    window.addEventListener('load', function() {
                ...
                xhr.onload = function(evt) {
                    ...
                    if (info['t']) {
                        ...
                        summary += (parseInt(info['t'])) > (new Date()).getHours()
                            ? '今日 ' + info['t'] + ':00 起发售 ' + (new Date(Date.now() + 11 * 86400* 1000)).toLocaleDateString() + ' 始发车票'
                            : '明日 ' + info['t'] + ':00 起发售 ' + (new Date(Date.now() + 12 * 86400* 1000)).toLocaleDateString() + ' 始发车票';
        ...
        if (count > 1) {
            for (var i = 1; i < count; i++) {
                ...
                tmp.push('</td><td>');
                tmp.push((new Date(Date.now() + (i + 11) * 86400* 1000)).toLocaleDateString());
                tmp.push('</td></tr>');
                detail += tmp.join('');
            }
        }
